...
func GetGoods() ([]map[string]string, error) {
	mysqlConn := common.GetMysqlConn()
	return mysqlConn.GetResults(mysql.Statement, "select id,name,bigimg,swcount,nums,price,spec,createtime,updatetime from goods ORDER BY swcount desc")
}

// GetPaginaGoods 获取分页商品
func GetPaginaGoods(paginaSQL string) ([]map[string]string, error) {
	mysqlConn := common.GetMysqlConn()
	sql := fmt.Sprintf("select id,name,bigimg,swcount,content,nums,price,spec,createtime,updatetime from goods ORDER BY swcount desc %s", paginaSQL)
